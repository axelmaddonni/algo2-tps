...
\Rep[estr\_dcnet]{
	...
	\comentario{4} 0 $<$ obtener($e$.computadoras, $c$).indice $<$ $\#$claves($e$.computadoras)-1 $\wedge$ 
...
destino($p$).ip $\in$ claves ($e$.computadoras) $\wedge$ 
	...
	\comentario{18} ($\exists$ $it$: itConj(paquete), $it$ $\in$ obtener($e$.computadoras, $c$).cola ) 
	Siguiente($it$) = $p$  ) )
}
...
\Function{iIniciarDCNet}{\In{r}{red}}{$\rightarrow res$: $estr\_dcnet$}
	...
	\While{HaySiguiente(itHostname)} \Comment{O(n) + O(1)}
		...
		\State X $\gets$ <index, Vacio(), Vacio(), Vacio(), 0> \Comment{O(1) + O(1) + O(1) + O(1) + O(1)}
	...
	\While{HaySiguiente(itPC)} \Comment{O($\#$Computadoras(r))}
		...
		\commentario{\comentario{$\#$Computadoras(r)) \'o	O(n)}}
...
\Function{iCrearPaquete}{\Inout{d}{estr\_dcnet}, \In{p}{paquete}}
	...
	\comentario{el paquete se agregar por copia, esto hace que la complejidad pedida no se cumpla??} 
