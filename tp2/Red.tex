...
\Function{iCopiar}{in r: estr\_red}{$\rightarrow$ res: red}
	...
	\While{HaySiguiente(itRed)} \Comment{O(1)} \Comment{se ejecuta n veces} \Comment{O(n)}
		\Status copiaAlcanzables $\gets$ Vacio() \Comment{diccionario} \Comment{O(1)}
		\Status itAlcanzables $\gets$ CrearIt(SiguienteSignificado(itRed).alcanzables) \Comment{O(1)}
		\While{HaySiguiente(itAlcanzables)} \Comment{Guarda: O(1)} \Comment{se ejecuta a lo sumo n veces} \Comment{O(n)}
			\Status copiaCaminos $\gets$ Vacia() \Comment{lista} \Comment{O(1)}
			\Status itCaminos $\gets$ CrearIt(SiguienteSignificado(itAlcanzables)) \Comment{O(1)}
			\While{HaySiguiente(itCaminos)}
				\Status AgregarAdelante(copiaCaminos, Siguiente(itCaminos)) \Comment{O(copiar(camino))}
				\Status Avanzar(itCaminos) \Comment{O(1)}
			\EndWhile			
			\Status Definir(copiaAlcanzables, SiguienteClave(itAlcanzables), copiaCaminos)
			\Status Avanzar(itAlcanzables) \Comment{O(1)}
		\EndWhile
		\Status Definir(res, SiguienteClave(itRed), Tupla(Copiar(SiguienteSignificado(itRed).interfaces), Copiar(SiguienteSignificado(itRed).conexiones, copiaAlcanzables)
		\Status Avanzar(itRed) \Comment{O(1)}
	\EndWhile
\EndFunction
\end{algorithmic}
\end{algorithm}
